...
class CPE {
    ...
    public function nota($comprobante, $detalle_comprobante) {
		global $model;
        $this->limpiar_generar();
        $array_items = array();
		...
		btrim(to_char(nrodocumento, '00000000'::text)) AS nota_documento_referencia FROM facturacion.detrebajas WHERE nrorebaja={$comprobante->idmovimiento}";
        ...
        $invoice
			...
			->setNumDocfectado($r->nota_documento_referencia) // Factura: Serie-Correlativo
            ...
            if($value->codtipodocumento == "03") {
                ...
                $detail->setTipoDoc($value->codtipodocumento)
                    ...
                    ->setDocReferencia((new Document())
                    	->setTipoDoc("03")
                    	->setNroDoc($r->nota_documento_referencia))
                    ->setEstado($value->dr_estado) // cat 19 1 -> adicionar, 2-> modificar, 3 -> anulado,
                //4 anulado en el dia antes de informar comprobante, transporte publico,
                //18/04/2022 ya no hay codigo 4 en el nuevo catalogo: usar codigo 3 nomas
                    // 08/05/2021:
